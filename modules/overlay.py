...
import config
from modules.hazard import HazardAssessment, HazardEngine, HazardLevel
...
class DashboardOverlay:
    ...
    def _draw_labels(self, frame, assessments):
        ...
        for ha in assessments:
            x1, y1, _x2, y2 = ha.actor.detection.bbox
            col = ha.bbox_color
            ...
            cv2.putText(
                frame,
                dist,
                (x1 + 2, min(y2 + 13, frame.shape[0] - 2)),
                font,
                0.35,
                col,
                1,
        ...
        if assessments and y + 20 < fh:
            ...
            for ha in sorted_actors:
                ...
                if ha.hazard_level == HazardLevel.CRITICAL:
                    cv2.rectangle(
                        sb, (2, y), (SIDEBAR_W - 2, y + row_h - 2), (40, 0, 0), -1
                    )
                elif ha.hazard_level == HazardLevel.WARNING:
                    cv2.rectangle(
                        sb, (2, y), (SIDEBAR_W - 2, y + row_h - 2), (30, 25, 0), -1
                    )
    ...
    def _draw_bev(self, assessments) -> np.ndarray:
        ...
        for lat_m, is_ego in [(0, True), (-3.5, False), (3.5, False)]:
            ...
            cv2.line(mm, (lx, 0), (lx, ego_y), col, 1)
        ...
        for ha in assessments:
            ...
            if ay < 4:
                ...
                cv2.arrowedLine(
                    mm, (ax_clamped, 14), (ax_clamped, 4), col, 1, tipLength=0.5
                )
                cv2.putText(
                    mm,
                    f"#{ha.actor.track_id}",
            ...
            cv2.putText(mm, tag, (tx, ay + 3), cv2.FONT_HERSHEY_SIMPLEX, 0.26, col, 1)

        # Ego triangle
        ev = np.array(
            [[ego_x, ego_y - 11], [ego_x - 6, ego_y + 7], [ego_x + 6, ego_y + 7]],
            np.int32,
        )
